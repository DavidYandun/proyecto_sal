...
public class ManagerPedido {
	...
	public List<TabVtsPedido> findAllPedidosEntregado(String fecha) {
		...
		sentenciaSQL = "SELECT p FROM TabVtsPedido p WHERE p.entregapedido=true AND  TO_CHAR(p.fechapedido,'DD-MM-YYYY')='"
				+ fecha + "'";
		q = em.createQuery(sentenciaSQL);
		listado = q.getResultList();
		return listado;
	...
	public TabVtsDetallePedido seleccionarDetallePedido(TabVtsPedido pedidoTmp, int index) {
		return pedidoTmp.getTabVtsDetallePedidos().get(index);
	}

	private void reemplazoDetallePedidoTmp(TabVtsDetallePedido pedidoOld, TabVtsDetallePedido pedidoNew) {
		pedidoNew.setIddp(pedidoOld.getIddp());
		pedidoNew.setCantidaddp(pedidoOld.getCantidaddp());
		pedidoNew.setValortotaldp(pedidoOld.getValortotaldp());
		pedidoNew.setValorunitariodp(pedidoOld.getValorunitariodp());
		pedidoNew.setSegundo(pedidoOld.getSegundo());
		pedidoNew.setLlevar(pedidoOld.getLlevar());
		pedidoNew.setTarjeta(pedidoOld.getTarjeta());
		pedidoNew.setTabVtsPedido(pedidoOld.getTabVtsPedido());
		pedidoNew.setTabVtsPlato(pedidoOld.getTabVtsPlato());
		pedidoNew.setObservaciondp(pedidoOld.getObservaciondp());
	}

	public void observacionDetallePedidoTmp(TabVtsPedido pedidoTmp, int iddp, String observacion) throws Exception {
		if (pedidoTmp == null)
			throw new Exception("Error primero debe crear un nuevo Pedido.");

		TabVtsDetallePedido pedidoOld = seleccionarDetallePedido(pedidoTmp, iddp);
		TabVtsDetallePedido pedidoNew = new TabVtsDetallePedido();
		reemplazoDetallePedidoTmp(pedidoOld, pedidoNew);
		pedidoNew.setObservaciondp(observacion);
		pedidoTmp.getTabVtsDetallePedidos().set(iddp, pedidoNew);
		calcularPedidoTmp(pedidoTmp);
	}

	public void sumarDetallePedidoTmp(TabVtsPedido pedidoTmp, int iddp) throws Exception {
		if (pedidoTmp == null)
			throw new Exception("Error primero debe crear un nuevo Pedido.");
		...
		double totalOld = Double.parseDouble(pedidoOld.getValortotaldp().toString());
		double unitarioOld = Double.parseDouble(pedidoOld.getValorunitariodp().toString());
		BigDecimal totalNew = BigDecimal.valueOf(totalOld + unitarioOld);

		reemplazoDetallePedidoTmp(pedidoOld, pedidoNew);
		pedidoNew.setCantidaddp(pedidoOld.getCantidaddp() + 1);
		pedidoNew.setValortotaldp(totalNew);

		pedidoTmp.getTabVtsDetallePedidos().set(iddp, pedidoNew);
		calcularPedidoTmp(pedidoTmp);
	}

	public void restarDetallePedidoTmp(TabVtsPedido pedidoTmp, int iddp) throws Exception {
		if (pedidoTmp == null)
			throw new Exception("Error primero debe crear un nuevo Pedido.");
