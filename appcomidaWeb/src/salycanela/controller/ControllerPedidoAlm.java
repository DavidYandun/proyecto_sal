...
public class ControllerPedidoAlm {
	...
	public void seleccionarDetalle(TabVtsDetallePedido detalle) {
		index = pedidoTmp.getTabVtsDetallePedidos().indexOf(detalle);
	}

	private int index;

	public void observacionDetalle() throws Exception {
		try {
			managerPedido.observacionDetallePedidoTmp(pedidoTmp, index, observacion);
			observacion = null;
			JSFUtil.crearMensajeInfo("Observación agregada");
	...
	public void sumarDetalle(int iddp) throws Exception {
		try {
			managerPedido.sumarDetallePedidoTmp(pedidoTmp, iddp);
			// JSFUtil.crearMensajeInfo("Detalle sumado correctamente " + iddp);
		} catch (Exception e) {
			JSFUtil.crearMensajeError(e.getMessage());
		}
	...
	public void restarDetalle(int iddp) throws Exception {
		try {
			managerPedido.restarDetallePedidoTmp(pedidoTmp, iddp);
			// JSFUtil.crearMensajeInfo("Detalle restado correctamente " + iddp);
		} catch (Exception e) {
			JSFUtil.crearMensajeError(e.getMessage());
		}
